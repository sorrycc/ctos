...
function transPackageJSON(c, p) {
  ...
  // set homepage
  if (!p.homepage) {
    var repo = c.repo || c.repository;
    if (repo) {
      p.homepage = 'https://github.com/' + repo;
    }
  }
  ...
  // set name
  // conflict packages
  var conflictMap = readJSON(path.join(__dirname, '../conflict.json'));
  if (conflictMap.indexOf(c.repo) > -1) {
    p.name = c.repo.replace('/', '-').toLowerCase();
  } else {
    p.name = c.name.toLowerCase();
  }

  // set repo
  p.repo = c.repo || c.repository;
